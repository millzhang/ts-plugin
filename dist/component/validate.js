...
define(["require", "exports", "../types/core"], function (require, exports, core_1) {
    ...
    function valid(value, type) {
        var result = false;
        switch (type) {
            case type = core_1.ValidateType['TELE']:
    ...
    function isTelephone(phone) {
        if (core_1.RegList.telephone.test(phone.toString())) {
            return true;
        }
        else {
            return false;
    ...
    function isEmail(email) {
        if (core_1.RegList.email.test(email.toString())) {
            return true;
        }
        else {
            return false;
    ...
    function isIdCard(idCard) {
        if (core_1.RegList.idCard.test(idCard)) {
            if (idCard.length == 18) {
                //将前17位加权因子保存在数组里
